...
function LinkedAccountsTab() {
  ...
  const handleLinkGoogle = async () => {
    try {
      ...
      logError("Link Google account failed", err);
  ...
  const handleUnlink = async (accountId: string, providerId: string) => {
    ...
    try {
      ...
      logError("Unlink social account failed", err, { accountId, providerId });
              ...
              {accounts?.find((acc: LinkedAccount) => acc.providerId === "google") ? (
                <button
                  onClick={() => {
                    ...
                    if (account) {
                      void handleUnlink(account.id, account.providerId);
                    }
                  }}
