...
export default function DocumentRepository() {
  ...
  const filterFolders = (nodes: FolderNode[], query: string): { nodes: FolderNode[], matchedIds: Set<string> } => {
    ...
    const filtered = nodes.reduce((acc, node) => {
      ...
      const childResult = node.children ? filterFolders(node.children, query) : { nodes: [], matchedIds: new Set() }
  ...
  const renderFolderTree = (nodes: FolderNode[], level = 0): JSX.Element[] => {
    const { nodes: filteredNodes, matchedIds } = level === 0 ? filterFolders(nodes, folderSearchQuery) : { nodes, matchedIds: new Set() }
  ...
  const columns = [
        ...
        return (
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {doc.company && (
                <p className="text-xs text-gray-600">{doc.company}</p>
              )}
            </div>
          </div>
        )
    ...
    },
    {
      ...
      render: (doc: Document) => (
        <span className="text-sm text-gray-900">{doc.modified}</span>
      )
    },
    {
      key: 'size',
    ...
    },
    {
      ...
      render: () => (
        <div className="flex items-center gap-2">
          ...
          <button className="p-1.5 hover:bg-gray-100 hover:text-red-600 rounded transition-colors" title="Delete">
      ...
      <div className="flex h-screen">
        ...
        >
          ...
          <div className="mt-8 pt-6 border-t border-gray-200">
            <h3 className="text-xs font-semibold text-gray-500 uppercase mb-3 px-2">{t.audit.quickFilters}</h3>
            <div className="space-y-1">
          ...
          >
            ...
            <div className="mb-6">
              ...
              <div className="flex items-center gap-2 text-sm text-gray-600 mb-4">
                {currentPath.map((path, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    {idx > 0 && <span>/</span>}
                    <button className={idx === currentPath.length - 1 ? 'text-blue-600 font-medium' : 'hover:text-gray-900'}>
                      {path}
                    </button>
                  </div>
                ))}
              ...
              <div className="flex items-center gap-4">
                <div className="flex-1 relative">
                  <Search className={`absolute ${locale === 'ar' ? 'right-3' : 'left-3'} top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400`} />
                  <input
                    type="text"
            ...
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              <div className="p-4 border-b border-gray-200">
                <p className="text-sm text-gray-600">{t.audit.showingDocuments.replace('{start}', '1').replace('{end}', '6').replace('{total}', '156')}</p>
              </div>
