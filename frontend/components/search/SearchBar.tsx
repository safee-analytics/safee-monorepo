...
export function SearchBar({ onOpenCommandPalette }: SearchBarProps) {
  ...
  const saveRecentSearch = (label: string) => {
    ...
    const updated = [newSearch, ...recentSearches.filter(s => s.label !== label)]
  ...
  return (
    <div className="relative flex-1 max-w-2xl">
      ...
      <div className="relative">
        <FiSearch className={`absolute ${locale === 'ar' ? 'right-4' : 'left-4'} top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400 pointer-events-none`} />
        <input
          ref={inputRef}
          type="text"
          value={query}
          ...
          className={`w-full ${locale === 'ar' ? 'pr-12 pl-24' : 'pl-12 pr-24'} py-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-safee-500 focus:bg-white transition-all`}
        />
        {/* Cmd+K Hint */}
        <button
          onClick={onOpenCommandPalette}
          className={`absolute ${locale === 'ar' ? 'left-3' : 'right-3'} top-1/2 transform -translate-y-1/2 hidden sm:flex items-center gap-1 px-2 py-1 text-xs font-medium text-gray-500 bg-white border border-gray-200 rounded hover:bg-gray-50 transition-colors`}
        >
            ...
            <div className="py-2">
              ...
              {recentSearches.map((recent) => (
                ...
                >
                  ...
                  <span className="text-sm text-gray-700">{recent.label}</span>
                </button>
              ))}
            </div>
          )}
                  ...
                  return (
                    ...
                    >
                      <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${
                        isEntity ? 'bg-purple-100' :
                        result.type === 'action' ? 'bg-safee-100' : 'bg-gray-100'
                      }`}>
                        <Icon className={`w-4 h-4 ${
                          isEntity ? 'text-purple-600' :
                          result.type === 'action' ? 'text-safee-600' : 'text-gray-600'
                        }`} />
                      </div>
                      <div className="flex-1 text-left">
                        <div className={`text-sm font-medium ${isEntity ? 'text-purple-900' : 'text-gray-900'}`}>
