        ...
        () =>
          void (async () => {
            try {
              ...
              await updateEmployee({ drizzle: ctx.drizzle, logger: ctx.logger }, employee.id, {
                odooEmployeeId,
              });
        ...
        () =>
          void (async () => {
            try {
              ...
              ctx.logger.error(
                { error: err, employeeId: employee.id },
                "Failed to sync employee update to Odoo",
              );
            }
          })(),
      );
          ...
          void (async () => {
            try {
              ...
              ctx.logger.error(
                ...
                "Failed to sync department to Odoo",
              );
            }
          })(),
      );
  ...
  public async syncDepartmentFromOdoo(
    ...
    const department = await syncDepartment(
      { drizzle: ctx.drizzle, logger: ctx.logger },
      {
        ...
        name: odooDepartment.complete_name ?? odooDepartment.name, // Use complete_name for full path
  ...
  public async syncAllDepartments(
    ...
    for (const odooDepartment of odooDepartments) {
      ...
      try {
        ...
        errors.push(
          `Failed to update department ${odooDepartment.name} relationships (pass 2): ${String(err)}`,
        );
      }
    }
